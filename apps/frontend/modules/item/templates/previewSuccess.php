				...
				<div class="content">
						...
						<div class="notify contact">
							...
							<div class="clr"></div>
                                                        ...
                                                        <?php endif; ?>        
							<div class="clr"></div>
							<p><strong>Цена:</strong><span><?php echo $item->getPrice() ?> Грн.</span></p>
							<div class="clr h"></div>
						...
						<div class="notify contact no-margin">
							<label for="pwd-inp">Пароль <strong>*</strong>:
                                                                <?php $class=($form['password']->hasError()) ? 'text validate error' : 'text'; echo $form['password']->render(array('class' => $class)); ?>      
                                                                 <span class="notify"><?php echo __('(пароль необходим для внесения изменений в объявление)')?></span>
                                                                 <span class="notify>"<?php echo $form['password']->renderError()?></span>
                                                        </label>
                                                                 
      
      <span class="notify"></span></label>
							<div class="clr"></div>
	                                                <?php echo $form['id'] ?>
      <label for="pwd2-inp"><?php echo __('Повторите')?> пароль <strong>*</strong>:							
                                                                <?php $class=($form['password_again']->hasError()) ? 'text validate error' : 'text'; echo $form['password_again']->render(array('class' => $class)); ?>
                                                               <span class="notify>"<?php echo $form['password_again']->renderError()?></span>
							</label>
							<div class="clr h"></div>
	<span class="message"><?php echo __('Размещая объявление, Я соглашаюсь с')?> <a href="javascript:winLoad('<?php echo url_for('service/rules') ?>')"><?php echo __('правилами и условиями')?></a> <?php echo __('портала Internetbazar, а также передаю все права на содержание этого объявления')?>.</span>
							<div class="clr h"></div>
                                                        <input type='button' class="btn add-2" value="<?php echo __('Изменить объявление')?>" onClick="location.href='<?php echo url_for('item/edit?id='.$item->getId())?>'">
                                                        <input type="submit" class="btn" value="<?php echo __('Подать объявление') ?>" name="submitbtn" />          							
						 </div>
					</form>
				</div>
				<div class="sidebar add checking">
	                           <?php include_partial('myimages_new_preview', array('item' => $item)) ?>	   
				</div>
				<div class="clr"></div>
			  </div>
                    </div>
